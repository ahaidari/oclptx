...
void SampleManager::ParseCommandLine(int argc, char** argv)
{
    ...
    if (_oclptxOptions.simple.value())
    {
        ...
        {
             NEWIMAGE::read_volume(_brainMask, _oclptxOptions.maskfile.value());
        }
        else
        {
             NEWIMAGE::read_volume(_brainMask, _oclptxOptions.seedref.value());
        }
...
void SampleManager::GenerateSeedParticles(float aSampleVoxel)
{
   ...
   Matrix seeds = MISCMATHS::read_ascii_matrix(_oclptxOptions.seedfile.value());
   ...
   float4 seed;
   //If there is no seed file given with the -x CLI parameter, we use the middle of the mask as the seed.
   if (seeds.Nrows() == 0)
   {
      seed.t = 1.0;
      ...
      cout << "Seeded at "<< "x = " << seed.x << " y= " << seed.y<< " z = "<< seed.z<<endl;
...
void SampleManager::GenerateSeedParticlesHelper(float4 aSeed, float aSampleVoxel)
{
   for (int p = 0; p<_nParticles; p++)
   {
     ...
     // cout<< " t = "<<randomParticle.t<< " x= "<< randomParticle.x << " y= " << randomParticle.y << " z= " << randomParticle.z<< endl;
     // cout<< " t = "<<rootVertex.t<< " x= "<< rootVertex.x << " y= " << rootVertex.y << " z= " << rootVertex.z<< endl;
   }
}

const volume<short int>* SampleManager::GetBrainMask()
{
     return &_brainMask;
}
